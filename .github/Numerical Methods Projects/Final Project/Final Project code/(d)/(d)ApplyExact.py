...
def NewB(P,d):
    for j in range(0,len(P)):
        for i in range(0,len(P)):
            x = i * d
            y = j * d
            P[0][i] = -math.log(2*(x**2)-2*x+1)/(3*math.log(2))
            P[len(P)-1][i] = -math.log(2*(x**2)-2*x+1)/(3*math.log(2))
            P[j][0] = -math.log(2*(y**2)-2*y+1)/(3*math.log(2))
            P[j][len(P)-1] = -math.log(2*(y**2)-2*y+1)/(3*math.log(2))
    return P


def ExactCheck(r,Pin ,d, x_c, y_c, i_l,i_h,i_max,j_max):
    New_Res = copy.deepcopy(Pin)
    ...
    for j in range(0,len(Pin)):
        for i in range(1,i_l[j]): 
            ...
            phi_exact = -(2/3) * math.log(math.sqrt(2)*r)/math.log(2)
            New_Res[j][i] = (Pin[j][i] - phi_exact)
        for i in range(i_h[j]+1,i_max): 
            x = i * d
            y = j * d
            r = math.sqrt((x-x_c)**2+ (y-y_c)**2)
            phi_exact = -(2/3) * math.log(math.sqrt(2)*r)/math.log(2)
            New_Res[j][i] = (Pin[j][i] - phi_exact)*d
...
def eploting(   e1,   d,    Kmax     ):
    ...
    for c in range(0,Kmax+1):
        x[c] = c
    plt.plot(x, e1, color = 'orangered', label = 'Error Compare to Exact Solution')
    plt.legend()
...
def main():
    ...
    for k in tqdm(range(0,Kmax+1)):
        ...
        r1 = ExactCheck(r,P ,d, x_c, y_c, i_l,i_h,i_max,j_max)
        #Res(r1,P,d,i_l,i_h,i_max,j_max)
