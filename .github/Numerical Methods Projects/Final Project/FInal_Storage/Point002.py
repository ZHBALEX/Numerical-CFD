...
def Point_Calculator_IBGSSOR(P, PgSOR, j, i, w, C_inner, C_total, C_xbl, C_xbh, C_ybl, C_ybh, C_edge, C_xlyl, C_xlyh, C_xhyl, C_xhyh):
    ...
    PgSOR[j][i] = \
        ...
        + C_inner[j][i]*((1-C_inner[j][i-1])*(P[j][i-1])+(1-C_inner[j][i+1])*(P[j][i+1])+(1-C_inner[j-1][i])*(P[j-1][i])+(1-C_inner[j+1][i])*(P[j+1][i])) 
    ...
    for j in range(1,j_max-1):
        for i in range(1, i_max-1):
            PgSOR[j][i] = Point_Calculator_IBGSSOR(P, PgSOR, j, i, w,\
                                               C_inner, C_total, C_xbl, C_xbh, C_ybl, C_ybh, C_edge, C_xlyl, C_xlyh, C_xhyl, C_xhyh)
    for j in range(j_max-1, j_max):
        for i in range(1, i_max):
            PgSOR[j][i] = \
                    1/4 * (  PgSOR[j][i-1] + PgSOR[j][i+1] + PgSOR[j-1][i] + PgSOR[j+1][i] )*w + (1-w)*PgSOR[j][i]
    for j in range(1,j_max):
        for i in range(i_max-1,i_max):
            PgSOR[j][i] = \
                    1/4 * (  PgSOR[j][i-1] + PgSOR[j][i+1] + PgSOR[j-1][i] + PgSOR[j+1][i] )*w + (1-w)*PgSOR[j][i]
        P = PgSOR
