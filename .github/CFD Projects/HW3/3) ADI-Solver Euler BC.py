...
class ADI_Solver:
    ...
    def BC(self, u , v):
        ...
        for j in range(0, self.j_max+1):
            u[0][j] = 1
            v[0][j] = 0
            u[self.i_max][j], v[self.i_max][j] = u[self.i_max-1][j], v[self.i_max-1][j]
        return u, v
    ...
    def UPdate(self, u, v, U,  Idoing, Ianother_max, flag):
        ...
        for ij in range(1,Ianother_max):
            ...
            d[ij] = U[i,j]-cox*((U[i+1,j]-U[i-1,j])/(2))\
                -coy * ((U[i,j+1]-U[i,j-1])/(2))\
                    +rxy * (U[i+flag[0],j+flag[1]]+U[i-flag[0],j-flag[1]])
...
def Vorticity(u, v, t):
    ...
    plt.colorbar()
    plt.title('Euler BC d={:.3f},t={:.2f}'.format(d,t))
    plt.xlabel('x')
    plt.ylabel('y')


    plt.tight_layout()
    plt.savefig('3Ed{:.3f}t{:.2f}.png'.format(d,t))
    plt.show()
