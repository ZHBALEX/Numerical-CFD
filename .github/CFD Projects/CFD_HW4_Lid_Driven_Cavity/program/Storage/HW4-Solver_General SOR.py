...
def abcd_uv( u,  U, V, u_ex, U_ex, V_ex,r,C):
    Res = 100
    u_star = np.copy(u)
    rows = np.size(u,0)
    cols = np.size(u,1)
...
def abcd_p(p, U_star, V_star, d, dt ):
        rows = np.size(p,0)
        cols = np.size(p,1)
        k = d/dt
        w = 1.7
        ...
        a = np.full(rows*cols, 1, dtype = 'float' )
        b = np.full(rows*cols,-2,dtype = 'float'  )
        c = a
        d = np.full(rows*cols,1,dtype = 'float'  )
        e = 
        for j in range(1, rows-1):
                for i in range(1,cols-1): # calculate d
...
def Assembler(input_numbr, a_value, b_value, c_value, e_value, f_value):
...
def solver(Re, u, v, p,  U, V, t, d, dt):
    ...
    for n in range(0, t_steps):
        ...
        rows, cols = np.size(u,0), np.size(u,1)

        a, b, c, d = abcd_uv( u,  U, V, u_ex, U_ex, V_ex,r, c)
        ...
        a, b, c, d = abcd_p(p, U_star, V_star, d, dt )
