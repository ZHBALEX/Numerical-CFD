...
'''
def Large_transfer(F, P, n, j_max, i_max):
...
def Large_transfer(F, n, j_max, i_max):
    ...
    P = [[0 for _ in range(large_i_max+1)] for _ in range(large_j_max+1)]
...
def OBdetector( j_max, i_max):
    P_OBin = [[0 for _ in range(0,j_max+1)] for _ in range(0,i_max+1)]

    P_OBleft = copy.deepcopy(P_OBin)
    P_OBright = copy.deepcopy(P_OBin)
    P_OBtop = copy.deepcopy(P_OBin)
    P_OBlow = copy.deepcopy(P_OBin)
    
    for j in range(1,j_max-1):
        for i in range(1,i_max-1):
            P_OBleft[j][1] = 1
            P_OBright[j][i_max-1] = 1
            P_OBtop[j_max-1][i] = 1
            P_OBlow[1][i] = 1
    return P_OBleft, P_OBright, P_OBtop , P_OBlow 



def CHANGEto1(C_in):
    for j in range(0, len(C_in)):
...
def LineSOR_Solver(P_input, i_max, j_max,  w ,d , \
    ...
    P_OBleft, P_OBright, P_OBtop , P_OBlow  = OBdetector( j_max, i_max)

    for j in range(0,j_max+1):
        for i in range(0,i_max+1):
            C_left[j][i] =  C_left[j][i] + P_OBleft[j][i]
            C_right[j][i] = C_right[j][i] + P_OBright[j][i]
            C_top[j][i] = C_top[j][i] + P_OBtop[j][i]
            C_low[j][i] = C_low[j][i] + P_OBlow[j][i]
    C_left = CHANGEto1(C_left)
    C_right = CHANGEto1(C_right)
    C_top = CHANGEto1(C_top)
    C_low = CHANGEto1(C_low)
    ...
    for i in range(0,i_max+1):
        for j in range(0,j_max):
            ...
            D[j][i] = -(P[j-1][i]*(1-C_top[j][i] ) + P[j+1][i]*(1-C_low[j][i] ) )*(1-C_inner[j][i]) +C_inner[j][i]  #- C_left[j][i] - C_right[j][i]              
            c[j][i] =(1- C_left[j][i])*(1-C_inner[j][i]) 
            a[j][i] = (1 - C_right[j][i])*(1-C_inner[j][i])
            b[j][i] = -4*(1-C_inner[j][i]) +C_inner[j][i] +C_left[j][i] +C_right[j][i]+C_top[j][i] +C_low[j][i]
    ...
    for j in range(1,j_max):
        ...
        for i in range(1, i_max):
            ...
            ''' \
        + (C_left[j][i]*P_new[j][i-1] +C_right[j][i]*P_new[j][i+1]\
              +C_low[j][i]*P_new[j-1][i]*(1-C_left[j][i])*(1-C_right[j][i])\
                  +C_top[j][i]*P_new[j+1][i]*(1-C_left[j][i])*(1-C_right[j][i]))*(1-C_inner[j][i]) #+0*C_inner[j][i]'''
    ...
    for i in range(1,i_max):
        for j in range(1,j_max):
            P[j][0] = P[j][1] +d*10
            P[j][i_max] = P[j][i_max-1] -d*10
            P[0][i] = P[1][i] 
            P[j_max][i] = P[j_max-1][i]

            D[j][i] = -(P[j-1][i] + P[j+1][i])*(1-C_inner[j][i]) +C_inner[j][i]- C_left[j][i] - C_right[j][i]              
            c[j][i] =(1- C_left[j][i])*(1-C_inner[j][i]) 
            a[j][i] = (1 - C_right[j][i])*(1-C_inner[j][i])
    ...
    for j in range(1,j_max):
        ...
        for i in range(1, i_max):
            ...
            P_new[j][0] = P_new[j][1] +d*10
            P_new[j][i_max] = P_new[j][i_max-1] -d*10
            P_new[0][i] = P_new[1][i] 
            P_new[j_max][i] = P_new[j_max-1][i]
...
def main():
    ...
    for k in tqdm(range(0,Kmax+1)):
        ...
        P = LineSOR_Solver(P_input, i_max, j_max,  w ,d, \
                 C_inner, C_total, C_xbl, C_xbh, C_ybl, C_ybh, C_edge, C_xlyl, C_xlyh, C_xhyl, C_xhyh, C_left , C_right, C_low, C_top)
