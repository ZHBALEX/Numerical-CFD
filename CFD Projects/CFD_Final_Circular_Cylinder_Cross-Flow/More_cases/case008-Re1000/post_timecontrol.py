...
def main():
    ...
    data_re, u_0, v_0, U_0, V_0 , \
        u_old_0, v_old_0, U_old_0, V_old_0,  p \
            = Recover_result(filename = 'result/save/output_N32_Re=1000_t=100.01_8x4.txt')
    ...
    for n in range(1,1001):
        ...
        print(time)
        print('%.2f'%time)
        filename = 'result/save/output_N32_Re=1000_t=%.2f_8x4.txt'%time
        print(f'{filename}')
        data_re, u_0, v_0, U_0, V_0 , \
        u_old_0, v_old_0, U_old_0, V_old_0,  p \
            = Recover_result(filename = 'result/save/output_N32_Re=1000_t=%.2f_8x4.txt'%time)
        
        p_e = p[:,:]*outer_e[:,:]
        p_w = p[:,:]*outer_w[:,:]
        p_n = p[:,:]*outer_n[:,:]
        p_s = p[:,:]*outer_s[:,:]
        drag_coeff[n] = (np.sum(p_w) - np.sum(p_e))*dy/(0.5*0.5)
        lift_coeff[n] = (np.sum(p_s) - np.sum(p_n))*dy/(0.5*0.5)
        print(n)
    print(drag_coeff)
