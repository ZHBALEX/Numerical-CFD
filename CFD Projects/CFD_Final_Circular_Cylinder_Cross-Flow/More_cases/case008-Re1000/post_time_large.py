...
def main():
    ...
    # # test ifluid
    # plt.imshow(outer_w+ outer_e -iS, cmap = 'gray')
    # plt.show()
    ...
    data_re, u_0, v_0, U_0, V_0 , \
        u_old_0, v_old_0, U_old_0, V_old_0,  p \
            = Recover_result(filename = 'result/save/output_N32.0_Re=1000.0_t=1.0_8.0x4.0222.txt')
    
    
    drag_coeff = np.zeros(101, dtype='float')
    lift_coeff = np.zeros_like(drag_coeff)
    for n in range(1,101):
        time = n
        print(time)
        print('%.2f'%time)
        filename = 'result/save/output_N32_Re=150_t=%.1f_8x4.txt'%time
        print(f'{filename}')
        data_re, u_0, v_0, U_0, V_0 , \
        u_old_0, v_old_0, U_old_0, V_old_0,  p \
            = Recover_result(filename = 'result/save/output_N32.0_Re=1000.0_t=%.1f_8.0x4.0222.txt'%time)
    ...
    time = np.linspace(0,101, np.size(drag_coeff))
    ...
    save_result('drag_coeff_8x4.txt', 'drag_coeff', drag_coeff)
    save_result('ligt_coeff_8x4.txt', 'lift_coeff', lift_coeff)
