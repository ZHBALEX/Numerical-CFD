...
def draw_streamline(u, v, len_x, len_y ,label):
    ...
    plt.savefig(f'result/stline_Ellip_N{label[0]}_Re={label[1]}_t={label[2]}_{len_x}x{len_y}.png')
    plt.close()
    # plt.show()
...
def draw_velocity_contour(u, v, len_x, len_y,label):
    ...
    plt.savefig(f'result/uv_Ellip_N{label[0]}_Re={label[1]}_t={label[2]}_{len_x}x{len_y}.png')
    plt.close()
    # plt.show()
...
def draw_u_contour(u, v, len_x, len_y ,label):
    ...
    plt.ylabel('Y')
    plt.title('u Contour')
    plt.savefig(f'result/u_Ellip_N{label[0]}_Re={label[1]}_t={label[2]}_{len_x}x{len_y}.png')
    plt.close()
    # plt.show()
...
def draw_p_contour(p, v, len_x, len_y ,label):
    ...
    plt.ylabel('Y')
    plt.title('u Contour')
    plt.savefig(f'result/p_Ellip_N{label[0]}_Re={label[1]}_t={label[2]}_{len_x}x{len_y}.png')
    plt.close()
    # plt.show()
...
def main():
    ...
    data_re, u_0, v_0, U_0, V_0 , \
        u_old_0, v_old_0, U_old_0, V_old_0,  p_0 \
            = Recover_result(filename = f'result/save/output_N32.0_Re=300.0_t=100.0_8.0x4.0222.txt')
    t_0 = data_re['t_0']
    Re = data_re['Re']
    Ly = data_re["Ly"]
    Lx = data_re["Lx"]
